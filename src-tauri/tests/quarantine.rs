...
fn make_temp_dir(prefix: &str) -> PathBuf {
  let root = std::env::temp_dir().join(format!("bypass-cleaner-it-{}-{}", prefix, Uuid::new_v4()));
...
fn quarantine_moves_file_and_writes_manifest() {
  ...
  let manifest_path = app_root.join(".quarantine").join("manifests").join(format!("{}.json", entry.id));
